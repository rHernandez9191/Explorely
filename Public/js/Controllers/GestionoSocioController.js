...
const RegistrarDatos = async () => {
  ...
  mostrarDescripcionEnDiv(sTipoDescripcion);
...
const ValidarDatos = (
  ...
  if (
    pPassConfirmacion == null ||
    pPassConfirmacion == "" ||
    pPassConfirmacion == undefined
  ) {
  ...
  if (
    pFechaConstitucion == "" ||
    pFechaConstitucion == null ||
    pFechaConstitucion == undefined ||
    new Date(pFechaConstitucion) >= new Date()
  ) {
    //resaltarLabelInvalido('lblConstitucion');
    resaltarInputInvalido("lblConstitucion");
    ImprimirMsjsError(
      "Por favor ingrese una fecha de constitucion menor a hoy"
    );
    return false;
  }
